...
export const commandBuilders = [
  ...
  new SlashCommandBuilder()
    ...
    .addSubcommand(command => command.setName('setup').setDescription('Configure verification').addChannelOption(option => option.setName('channel').setDescription('Verification channel').setRequired(true).addChannelTypes(ChannelType.GuildText)).addRoleOption(option => option.setName('verified-role').setDescription('Role granted after verification').setRequired(true)).addRoleOption(option => option.setName('unverified-role').setDescription('Role removed after verification')).addStringOption(option => option.setName('mode').setDescription('Verification mode').setRequired(true).addChoices({ name: 'Button', value: 'button' }, { name: 'Discord OAuth', value: 'oauth' })).addStringOption(option => option.setName('message').setDescription('Panel message').setMaxLength(2_000)))

    ...
    <Shell>
      <section className="section server-directory">
        <div className="section-head server-directory-head">
          <div><span className="section-kicker">SERVERS</span><h1>Choose a server</h1><p>Only servers where you can manage settings are shown.</p></div>
        ...
        {state.error === 'signin' && <div className="card"><h2>Sign in to continue</h2><p>Discord confirms which servers you can manage. ModerationDesk never receives your password.</p><a className="button" href="/api/auth/login">Continue with Discord</a></div>}
          ...
          <div className="server-grid">
            {state.guilds.map(guild => (
              <article className="card guild-card" key={guild.id}>
                ...
                <div className="grow"><div className="guild-title"><h3>{guild.name}</h3>{guild.installed && <span className="connection online">Connected</span>}</div>{guild.installed && <p>Open this server’s settings.</p>}</div>

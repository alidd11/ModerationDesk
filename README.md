...
ModerationDesk now protects against destructive channel and role activity, risky permission escalation, ban/kick floods, webhook changes, bot additions and server setting changes. Pro+ servers can choose per-event thresholds, a containment action, an optional quarantine role, restoration for deleted channels and roles, and an immediate lockdown response.
...
- Discord guild subscriptions: Free, Pro ($3.99/server/month) and Pro+ ($7.99/server/month)
...
See `DEPLOYMENT.md` for the exact Railway, Vercel and Discord setup.

For the two Discord Store products, benefits and Railway variables, see [Discord guild subscriptions](docs/DISCORD_GUILD_SUBSCRIPTIONS.md).
